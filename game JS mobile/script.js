...
const generateNormalBlock = () => {
    ...
    let randomPosition = Math.floor(Math.random() * 375);
    ...
    setInterval( () => {
        ...
        if(blockTop >= 730 && blockTop < 770 && blockLeft + 20 > playerPosition && blockLeft < playerPosition + playerObj.width){
            playerObj.lives--;
            block.remove();
            lives.innerHTML = '';
            for(let i = 0; i < playerObj.lives; i++){
...
const generateCrazyBlock = () => {
    ...
    setInterval( () => {
        ...
        let blockLeft = parseInt(window.getComputedStyle(block).left);
        let playerPosition = parseInt(window.getComputedStyle(player).left);
        if(blockTop >= 740 && blockTop < 760 && blockLeft + 10 > playerPosition && blockLeft < playerPosition + playerObj.width){
            playerObj.lives--;
            block.remove();
            lives.innerHTML = '';
            for(let i = 0; i < playerObj.lives; i++){
